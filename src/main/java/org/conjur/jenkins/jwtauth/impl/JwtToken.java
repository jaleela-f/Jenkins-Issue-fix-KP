...
public class JwtToken {
    ...
    public static JwtToken getUnsignedToken(String pluginAction, Object context) {
        ...
        if(contextObject instanceof Jenkins)
        {
        	//Jenkins.get();
			LOGGER.log(Level.FINE, "Inside contextObject is Jenkisn"+Jenkins.getInstanceOrNull());
			  ItemGroup itemGroup = Jenkins.get().getItemGroup();
        	
        	 ModelObject item = (ModelObject) contextObject;
        	//jwtToken.claim.put("jenkins_name", contextObject.getDisplayName());
        	LOGGER.log(Level.FINE, "Inside contextObject is Jenkisn"+Jenkins.get().allItems());
        	 jwtToken.claim.put("jenkins_full_name", Jenkins.get().getFullName());
             jwtToken.claim.put("jenkins_name",itemGroup.getDisplayName());
             
           
             LOGGER.log(Level.FINE,itemGroup.getDisplayName());
             //Item item1 = (Item) itemGroup;
             ...
             if (parent != null && parent instanceof AbstractItem) {
                 ...
                 jwtToken.claim.put("jenkins_parent_full_name", item1.getFullName());
                 jwtToken.claim.put("jenkins_parent_name", item1.getName());
                 jwtToken.claim.put("jenkins_parent_task_noun", ((AbstractItem) item1).getTaskNoun());
                 item =  (AbstractItem) parent;

                 
                 if (item instanceof ItemGroup) {

                     ItemGroup itemGroup2 = (ItemGroup) item;
        	...
        	// Add identity field
             LOGGER.log(Level.FINE,"Identity field value"+globalConfig.getIdentityFormatFieldsFromToken());
