...
public interface ConjurSecretCredentials extends StandardCredentials {
	...
	default Secret secretWithConjurConfigAndContext(ConjurConfiguration conjurConfiguration, ModelObject context) {
		LOGGER.log(Level.FINE, "secretWithConjurConfigAndContext>>");
	...
	static ConjurSecretCredentials credentialWithID(String credentialID, ModelObject context) {
		...
		getLogger().log(Level.FINE, "* CredentialID with credentialWithID: {0}", credentialID);

		if (context != null) {
			// getLogger().log(Level.FINE, "* Context Id: {0}", context.getDisplayName());
			// LOGGER.log(Level.FINE, "* Context Id not null>>>:" +
		...
		LOGGER.log(Level.FINE, "* Context Id in credentialWithID>>>:" + context);
	...
	static Secret getSecretFromCredentialIDWithConfigAndContext(String credentialID,
		...
		getLogger().log(Level.FINE,"after fetching credential"+credential);
		
		if (credential != null) {
			getLogger().log(Level.FINE, "Credential not null" + credential);
			if(credential instanceof ConjurSecretCredentialsImpl)
			{
				getLogger().log(Level.FINE, "Credential instance of ConjurSecretCredentialsImpl " + credential);
				return null;
			}else {
				getLogger().log(Level.FINE, "Credential not an instance of ConjurSecretCredentialsImpl " + credential);
			return credential.secretWithConjurConfigAndContext(conjurConfiguration, effectiveContext);
			}
			
		} else {
			getLogger().log(Level.FINE, "Credential is null");
			return null;
		}
	}
