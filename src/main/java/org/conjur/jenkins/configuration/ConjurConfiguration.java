...
public class ConjurConfiguration extends AbstractDescribableImpl<ConjurConfiguration> implements Serializable {
	...
	private static ListBoxModel fillCredentialIDItemsWithClass(Item item, String credentialsId, Class<? extends StandardCredentials> credentialClass) {
		...
		LOGGER.log(Level.FINE,"Iniside ListBoxModel to populate the Credentials"+item);
		if (item == null && !Jenkins.get().hasPermission(Jenkins.ADMINISTER)) {
			LOGGER.log(Level.FINE, "Item is null for fil Credentials");
			//result.add("Test1");
		...
		if (item != null
			...
			LOGGER.log(Level.FINE, "Item is not null for fil Credentials"+item.getDisplayName());
			//result.add("Test1");
			return result.includeCurrentValue(credentialsId);
			//return result;
		//return result.includeCurrentValue(credentialsId);
